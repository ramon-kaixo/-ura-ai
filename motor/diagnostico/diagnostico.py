"""Diagnostico — motor de diagnóstico del sistema."""
...
class Diagnostico:
    ...
    def __init__(self, config: UraConfig, qdrant: QdrantClient, executor: SubprocessExecutor | None = None) -> None:
        self.config = config
        self.qdrant = qdrant
    ...
    def _tomar_snapshot_inicial(self) -> dict:
        ...
        try:
            r = self.executor.run(["ps", "-eo", "pid,comm", "--sort=-pid"], timeout=3)
