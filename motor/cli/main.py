import argparse, sys, json, logging, subprocess, socket
...
def _cmd_pipeline(config: UraConfig, args):
...
def _cmd_scan(config: UraConfig):
...
def _cmd_diagnose(config: UraConfig):
...
def _cmd_status(config: UraConfig):
...
def _cmd_check(config: UraConfig):
...
def _cmd_verify(config: UraConfig):
...
def _cmd_history(config: UraConfig):
...
def _cmd_trend(config: UraConfig):
...
def _cmd_graph(config: UraConfig):
...
def _cmd_perf(config: UraConfig):
...
def _cmd_calibrate(config: UraConfig, args):
...
def _cmd_cross(config: UraConfig):
...
def _cmd_detect(config: UraConfig):
...
def _cmd_health_check(config: UraConfig):
...
def _cmd_qdrant_backup(config: UraConfig):
...
def _cmd_summarise(config: UraConfig):
...
def _cmd_learn(config: UraConfig):
...
def _cmd_notify(config: UraConfig):
