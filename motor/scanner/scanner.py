"""Scanner — escáner principal del sistema."""
...
class Scanner:
    ...
    def __init__(self, config: UraConfig, executor: SubprocessExecutor | None = None) -> None:
        self.config = config
    ...
    def _es_fisico(self) -> bool:
        try:
            r = self.executor.run(["systemd-detect-virt"], timeout=5)
    ...
    def _check_servicios(self) -> dict:
        ...
        for svc in SERVICIOS_SYSTEMD:
            try:
                r = self.executor.run(["systemctl", "is-active", svc], timeout=5)
                out = r.stdout.strip()
    ...
    def _detectar_orphans(self) -> list:
        ...
        try:
            r = self.executor.run(["docker", "images", "-f", "dangling=true", "-q"], timeout=10)
