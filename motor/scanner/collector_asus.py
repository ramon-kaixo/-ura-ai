...
PUERTO_QDRANT = 6333
PUERTO_WHISPER = 9090
OPCIONES_SSH = ["-o", "ConnectTimeout=5", "-o", "StrictHostKeyChecking=accept-new",
                "-o", "BatchMode=yes"]
...
def _check_ollama(host: str) -> bool:
    ...
    try:
        r = subprocess.run(["curl", "-sf", f"http://{host}:{PUERTO_OLLAMA}/api/tags"],
...
def _check_qdrant(host: str) -> bool:
    ...
    try:
        r = subprocess.run(["curl", "-sf", f"http://{host}:{PUERTO_QDRANT}/collections"],
                         capture_output=True, timeout=5)
        return r.returncode == 0
    except Exception as e:
...
def _check_whisper(host: str) -> bool:
    ...
    try:
        r = subprocess.run(["curl", "-sf", f"http://{host}:{PUERTO_WHISPER}/health"],
                         capture_output=True, timeout=5)
        return r.returncode == 0
    except Exception as e:
...
def _check_temp(host: str) -> float:
    ...
    try:
        ...
        r = subprocess.run(["ssh", *OPCIONES_SSH, target,
