...
class FileExporter(_SpanEventSink):
    ...
    def _open_file(self) -> None:
        ...
        base = os.path.join(self._path, "trace")
        while os.path.exists(f"{base}.{self._file_index}.jsonl"):
            self._file_index += 1
    ...
    def _write_event(self, event: SpanEvent) -> None:
        ...
        if self._event_count >= 1000 and self._file is not None:
            try:
                fsize = os.path.getsize(self._file.name)
...
class TraceExporter(_SpanEventSink):
    ...
    def _next_path(self) -> str:
        ...
        while os.path.exists(f"{self._path}.{self._file_index}"):
            self._file_index += 1
