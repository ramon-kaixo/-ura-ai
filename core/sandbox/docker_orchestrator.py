...
MEM="2g"; CPU="2"; TIMEOUT=30; SKILL_TO=5; IMG="python:3.12-slim"
TD = BASE_DIR / "TOOLS" / "tests"

@dataclass
class ResultadoSandbox:
    ok: bool; ejecuto: bool; pasados: int; fallidos: int; fallos: list[str]
    stdout: str; stderr: str; tiempo_ms: float; ram_mb: float; error: str | None
    ...
    def resumen(s):
        ...
        if s.error: r.append(f"  ERROR: {s.error}")
        return "\n".join(r)

class DockerOrchestrator:
    def __init__(s, td=TD): s._td=td
    async def validar(s, codigo, nombre):
        if not s._docker(): return ResultadoSandbox(False,False,0,0,[],"","",0,0,"Docker no disponible")
        with tempfile.TemporaryDirectory() as d: return await s._run(Path(d), codigo, nombre)
    async def _run(s, d, cod, nom):
        t0=asyncio.get_event_loop().time()
        (d/"skills").mkdir(); (d/"skills"/f"{nom}.py").write_text(cod)
        td=d/"tests"
        if s._td.exists(): shutil.copytree(s._td, td)
        else: td.mkdir(); (td/"ts.py").write_text("def t(): assert True")
        (d/"Dockerfile").write_text(s._df(cod,nom)); (d/"rv.py").write_text(s._rv(nom))
        ...
        try:
            ...
            if b.returncode: return ResultadoSandbox(False,False,0,0,[],"",be.decode(errors="ignore")[:2000],0,0,"build fail")
            rp=await asyncio.create_subprocess_exec("docker","run","--rm",f"--memory={MEM}",f"--cpus={CPU}","--network=none","--read-only","--tmpfs","/tmp:64m",tag,stdout=-1,stderr=-1)
            try:
                so,se=await asyncio.wait_for(rp.communicate(),TIMEOUT)
            except asyncio.TimeoutError:
                rp.kill(); return ResultadoSandbox(False,False,0,0,["TIMEOUT"],"","",TIMEOUT*1000,0,"timeout")
            t1=asyncio.get_event_loop().time(); ms=(t1-t0)*1000
            s2=so.decode(errors="ignore").strip(); s3=se.decode(errors="ignore").strip()
            dt={}
