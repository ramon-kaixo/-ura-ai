...
from core.event_bus import publish, ensure_publisher
...
def _get_processed_files() -> set[str]:
    ...
    try:
        ...
        log.debug("silent exception in data_analyzer")
    conn.close()
    return files
    ...
    for raw_path in raw_files:
        ...
        try:
            ...
            for source in by_source:
                publish("analytics", {"source": source, "file": fname, "records": len(by_source[source])})
            for source in by_source:
                publish("analytics", {"source": source, "file": fname, "records": len(by_source[source])})
