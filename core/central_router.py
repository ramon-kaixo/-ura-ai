...
class CentralRouter:
    ...
    async def process_request(self, texto: str) -> dict:
        ...
        # PRIMERO: Consultar a AgenteMaestro si es introspección
        try:
            from core.agente_maestro import get_agente_maestro

            maestro = get_agente_maestro()
            respuesta_maestro = maestro.preguntar(texto)

            # Si AgenteMaestro devolvió una respuesta válida (no es el mensaje de error)
            if respuesta_maestro and "No estoy seguro" not in respuesta_maestro:
                self._log("REQUEST", "Intent: introspeccion (via AgenteMaestro)")
                duration_ms = int((time.time() - t_start) * 1000)
                obs_logger.log_ok(
                    respuesta_maestro[:500], trace_id=trace_id, duracion_ms=duration_ms
                )
                self.observability.agregar_trace(
                    trace_id, "central_router", "process_request", duration_ms
                )
                return {
            ...
            logger.warning(f"Error consultando AgenteMaestro: {e}, usando sistema normal")

        # SEGUNDO: Si no es introspección, usar sistema de clasificación por keywords
        intent, confidence = self._detect_intent_keywords(texto)

        # UMBRAL DE CONFIANZA MÍNIMO: 0.55
        if confidence < 0.55:
            duration_ms = int((time.time() - t_start) * 1000)
        ...
        # Registrar interacción en unified logger
        self.unified_logger.log_agent_interaction(
            agent=agent_path,
            intent=intent,
            texto=texto,
            response=response,
        ...
        self._log("REQUEST", f"Intent: {intent}, Agent: {agent_path}, Confidence: {confidence:.2f}")
        ...
        try:
            ...
            if agent_path == "system":
                ...
                if hasattr(agent, "execute"):
                    result_dict = self._call_agent_with_timeout(agent_name, agent.execute, texto)
                # Prioridad 2: Método procesar()
                elif hasattr(agent, "procesar"):
                    result_dict = self._call_agent_with_timeout(agent_name, agent.procesar, texto)
                # Prioridad 3: Método ejecutar()
                elif hasattr(agent, "ejecutar"):
                    result_dict = self._call_agent_with_timeout(agent_name, agent.ejecutar, texto)
                # Prioridad 4: Método consultar()
                elif hasattr(agent, "consultar"):
                    result_dict = self._call_agent_with_timeout(agent_name, agent.consultar, texto)
                # Prioridad 5: Método responder()
                elif hasattr(agent, "responder"):
                    result_dict = self._call_agent_with_timeout(agent_name, agent.responder, texto)
                # Prioridad 6: Método run()
                elif hasattr(agent, "run"):
                    result_dict = self._call_agent_with_timeout(agent_name, agent.run, texto)
                # Prioridad 7: Método search()
                elif hasattr(agent, "search"):
                    result_dict = self._call_agent_with_timeout(agent_name, agent.search, texto)
                # Prioridad 8: Método process()
                elif hasattr(agent, "process"):
                    result_dict = self._call_agent_with_timeout(agent_name, agent.process, texto)
    ...
    async def execute(self, intent: str, texto: str, confidence: float = 0.5) -> str:
        ...
        result = await self.agent_execute(agent_path, texto)
    ...
    async def _execute_with_fallback(
        self, intent: str, texto: str, agent_path: str, error: str
    ) -> str:
        """
        ...
        """
        try:
            # Importar y ejecutar agente con lógica específica
            module_path, class_name = agent_path.rsplit(".", 1)
            module = __import__(module_path, fromlist=[class_name])
            agent_class = getattr(module, class_name)
            agent = agent_class()

            # Llamar método apropiado según agente
            ...
            # Fallback final: intentar métodos genéricos
            ...
            # Sistema de degradación inteligente: buscar agente similar
            similar_intent = self._find_similar_agent(intent)
            if similar_intent:
                similar_path = self.intent_to_agent.get(similar_intent)
                self._log(
                    "DEGRADATION", f"Intent original: {intent}, Degradando a: {similar_intent}"
                )

                # Registrar degradación en unified logger
                ...
                try:
                    ...
                    if hasattr(agent, "procesar"):
                        response = agent.procesar(texto)
                        return f"[Degradación] Agente original ({intent}) no disponible. Usando agente alternativo ({similar_intent}).\n\n{response}"
                    elif hasattr(agent, "ejecutar"):
                        response = agent.ejecutar(texto)
                        return f"[Degradación] Agente original ({intent}) no disponible. Usando agente alternativo ({similar_intent}).\n\n{response}"
                    else:
                        return f"[Degradación] Agente original ({intent}) no disponible. Usando agente alternativo ({similar_intent})."
            ...
            return f"Lo siento, hubo un error procesando tu solicitud: {error_msg}"
    ...
    def check_maleta_cache(self, intent: str, query: str) -> str | None:
        ...
        # Mapear intent a domain (93 agentes)
        intent_to_domain = {
            # COCINA (12 agentes)
        ...
        # Obtener maletas promocionadas del dominio
        ...
        # Buscar maleta con mejor match de similitud
        best_maleta = None
        best_similarity = 0.0
        ...
        for maleta in domain_maletas:
            ...
            # Calcular similitud con queries existentes
            ...
            if self.embedding_service:
                ...
                for q, r in zip(queries, responses, strict=False):
                    q_lower = q.lower()
                    common_words = set(query_lower.split()) & set(q_lower.split())
    ...
    def _find_similar_agent(self, failed_intent: str) -> str | None:
        ...
        # Mapeo de categorías (basado en la estructura de intent_to_agent)
        category_map = {
            # COCINA
            "cocina_espanola": "cocina",
            "cocina_navarra": "cocina",
        ...
        # Buscar agentes de la misma categoría
        candidates = []
        ...
        for intent, keywords in self.intent_keywords.items():
            ...
            current_category = category_map.get(intent)
            if current_category == failed_category:
                # Misma categoría: buscar por similitud de keywords
                current_keywords = set(keywords)
                similarity = len(failed_keywords & current_keywords)

                candidates.append((intent, similarity))

        # Ordenar por similitud y devolver el mejor, o el primero si no hay similitud
        if candidates:
...
def _build_intent_to_agent():
    return {
        # GOBERNANZA (1 agente)
        "introspeccion": "core.agente_maestro.AgenteMaestro",
        # COCINA (12 agentes)
        ...
        # VISIÓN/GUI (2 agentes)
        "vision": "agents.agente_vision.AgenteVision",
        "gui": "agents.agente_gui.GUIAgent",
        # ORQUESTACIÓN (3 agentes)
        "busqueda": "core.buscadores.orchestrator.SearchOrchestrator",
        "orquestador_documentacion": "agents.agente_orquestador_documentacion.AgenteOrquestadorDocumentacion",
        # EXISTENTES
        "sistema": "system",
        "explorar": "core.explorador_sistemico.ExploradorSistemico",
    }
