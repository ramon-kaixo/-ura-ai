...
def cmd_doctor(args) -> int:
    ...
    try:
        ...
        report(
            "OK" if v == SCHEMA_VERSION else "FAIL",
            "schema_version",
            f"v{v}" + (f" (expected v{SCHEMA_VERSION})" if v != SCHEMA_VERSION else ""),
        )
        ...
        report(
            ...
            "migrations",
            f"{len(applied)}/{len(MIGRATIONS)} aplicadas" if not pending else f"Pendientes: {pending}",
        )
        ps = conn.execute("SELECT COUNT(*) as c FROM op_vector_sync WHERE status IN ('pending','failed')").fetchone()[
            "c"
        ]
        ...
        report(
            "WARN" if orph else "OK",
            "orphan_vectors",
            f"{len(orph)} pendientes de eliminar" if orph else "Sin vectores hu\u00e9rfanos",
        )
        qc = _get_qdrant()
        ...
        report(
            "WARN" if pc > 0 and fc > pc * 0.2 else "OK",
            "vacuum",
            f"DB sin fragmentar ({fc}/{pc})" if fc <= pc * 0.2 else f"Fragmentada: {fc}/{pc}",
        )
    finally:
        conn.close()
