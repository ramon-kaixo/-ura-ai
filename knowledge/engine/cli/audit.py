...
def _audit_integrity(conn, report) -> None:
...
def _audit_orphans(conn, report) -> None:
...
def _audit_active_version(conn, report) -> None:
...
def _audit_stuck_jobs(conn, report) -> None:
    stuck = conn.execute(
...
def _audit_wal(conn, db_path, report) -> None:
...
def _audit_pending_sync(conn, report) -> None:
...
def _audit_backend(report) -> None:
    try:
...
def _audit_disk(db_path, report) -> None:
    import shutil
...
def cmd_audit_db(args) -> int:
    ...
    conn = open_db(db_path)
    _audit_integrity(conn, report)
    _audit_orphans(conn, report)
    _audit_active_version(conn, report)
    _audit_stuck_jobs(conn, report)
    _audit_wal(conn, db_path, report)
    _audit_pending_sync(conn, report)
    _audit_backend(report)
    _audit_disk(db_path, report)
