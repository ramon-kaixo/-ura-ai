...
async function send() {
    ...
    try {
        ...
        let j = await r.json();
        d.innerHTML += `<p><b>${currentDest.charAt(0).toUpperCase() + currentDest.slice(1)}:</b> ${j.respuesta || j.response || j.error || 'Esperando...'}</p>`;
    } catch(e) {
...
def api_metrics():
    ...
    try:
        ...
        # Temperatura (macOS: via `pmset -g therm` o `osx-cpu-temp`)
        temperature = None
        try:
            import subprocess
            ...
            for line in r.stdout.splitlines():
                if "CPU_Scheduler_Limit" in line:
                    ...
                    # Convertir el limite termico a temperatura estimada
                    temp = 100 - (100 - 30) * (1 - int(val) / 100) if val.isdigit() else None
                    if temp:
                        temperature = round(temp, 1)
                    break
            if temperature is None and os.path.exists("/usr/local/bin/osx-cpu-temp"):
                r = subprocess.run(["osx-cpu-temp"], capture_output=True, text=True, timeout=3)
                temp_str = r.stdout.strip().replace("°C", "")
...
def api_chat():
    ...
    try:
        ...
        respuesta = f"Hola {data.get('destination', '')}! Soy URA. El enrutador IA no esta activo ahora, pero el panel funciona. Puedo mostrarte: estado del sistema, metricas, camaras, o conectar con los agentes via Bus. ¿Que necesitas?"
...
def _route_command(message: str) -> dict | None:
    ...
    # --- BUSQUEDA DE VIDEOS ---
    if msg.startswith("busca videos de ") or msg.startswith("busca videos "):
        query = msg.replace("busca videos de ", "").replace("busca videos ", "").strip()
        if query:
            import subprocess
            import json
            import os
            ...
            output_file = lines[-1] if lines else ""
            if output_file and Path(output_file).exists():
                with open(output_file) as f:
                    data = json.load(f)
                resp = [f"🎥 {len(data)} videos encontrados para '{query}':", ""]
                for i, v in enumerate(data[:6]):
                    resp.append(f"[{i + 1}] {v.get('titulo', 'sin titulo')[:80]}")
                    resp.append(f"    Puntuacion: {v.get('score', '?')} | {v.get('url', '')[:80]}")
                    resp.append("")
                resp.append('Di "guarda 1" o "guarda 1,3,5" para descargar')
                return {
                    "ok": True,
                    "response": "\n".join(resp),
                    "meta_confidence": 1.0,
    ...
    if msg.startswith("guarda ") and any(c.isdigit() for c in msg):
        seleccion = msg.replace("guarda ", "").strip()
        import subprocess

        cand_file = "/tmp/ura_videos_*.json"
        import glob

        files = sorted(glob.glob(cand_file), key=os.path.getmtime, reverse=True)
        if files:
            subprocess.run(
                [
                    "python3",
                    str(APP_PATH / "agents" / "agente_seleccion_video.py"),
                    files[0],
                    seleccion,
                ],
                capture_output=True,
            ...
            return {
                "ok": True,
                "response": "✅ Videos guardados en knowledge/media/",
                "meta_confidence": 1.0,
                "value_score": 0.5,
            }
