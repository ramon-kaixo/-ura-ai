...
class SecurityValidator:
    ...
    def _is_in_allowed_dir(self, path: str) -> bool:
        ...
        allowed_dirs = self.config.get("allowed_temp_dirs", []) + self.config.get(
            "allowed_log_dirs", []
        )
    ...
    def clean_pip_cache(self) -> float:
        ...
        try:
            ...
            if pip_cache_dir.exists():
                before_size = sum(
                    f.stat().st_size for f in pip_cache_dir.rglob("*") if f.is_file()
                ) / (1024**3)

            result = subprocess.run(
                ["pip3", "cache", "purge"], capture_output=True, text=True, timeout=300
            )
...
class MacCleaner(SystemCleaner):
    ...
    def clean_brew_cache(self) -> float:
        ...
        try:
            ...
            result = subprocess.run(
                ["brew", "cleanup", "--prune=all"], capture_output=True, text=True, timeout=300
            )
...
class CodeQualityChecker:
    """Ejecuta herramientas de calidad de código sobre el repositorio URA"""

    def __init__(self):
        self.ura_root = os.environ.get("URA_ROOT", os.path.expanduser("~/URA/ura_ia_1972"))
        self.venv_python = os.path.join(self.ura_root, "venv", "bin", "python3")
        self.results = {}
        self.env = os.environ.copy()
        npm_global = os.path.expanduser("~/.npm-global/bin")
        if os.path.isdir(npm_global) and npm_global not in self.env.get("PATH", ""):
            self.env["PATH"] = f"{npm_global}:{self.env.get('PATH', '')}"

    def _run_tool(
        self, tool: str, args: list[str], cwd: str | None = None, python_module: bool = True
    ) -> dict:
        """Ejecuta una herramienta y captura salida"""
        if python_module and os.path.exists(self.venv_python):
            cmd = [self.venv_python, "-m", tool] + args
        else:
            cmd = [tool] + args
        try:
            result = subprocess.run(
            ...
            logger.warning(f"Herramienta {tool} no instalada")
            return {"tool": tool, "exit_code": -1, "ok": False, "error": f"{tool} not found"}
        except subprocess.TimeoutExpired:
            logger.warning(f"Herramienta {tool} timeout")
            return {"tool": tool, "exit_code": -1, "ok": False, "error": "timeout"}

    def run_all(self) -> dict:
        """Ejecuta todas las herramientas de calidad"""
        logger.info("Iniciando análisis de calidad de código...")
        checks = {}

        checks["ruff_check"] = self._run_tool(
            "ruff", ["check", "--fix", "--unsafe-fixes", self.ura_root]
        )
        checks["ruff_format"] = self._run_tool("ruff", ["format", self.ura_root])
        checks["jscpd"] = self._run_tool(
            "jscpd",
            [self.ura_root, "--min-lines", "5", "--min-tokens", "50", "--gitignore"],
            python_module=False,
        )
        checks["vulture"] = self._run_tool("vulture", [self.ura_root, "--min-confidence", "70"])
        checks["radon_cc"] = self._run_tool(
            "radon",
            [
                "cc",
                self.ura_root,
                "-s",
                "--exclude",
                "venv/*,.tox/*,__pycache__/*,.mypy_cache/*,.git/*",
            ],
        )

        ok_count = sum(1 for c in checks.values() if c.get("ok"))
        total = len(checks)
        logger.info(f"Calidad de código: {ok_count}/{total} herramientas OK")
...
class MaintenanceOrchestrator:
    ...
    def run_maintenance(self) -> dict:
        ...
        # Calidad de código (siempre)
        code_checks = self.code_quality.run_all()
        ...
        self.results = {
            ...
            "code_quality": {k: v.get("ok") for k, v in code_checks.items()},
            "timestamp": datetime.now().isoformat(),
        }
    ...
    def _save_results(self):
        ...
        results_file = (
            LOG_DIR / f"maintenance_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        try:
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Mantenimiento completado. Espacio liberado: {results['space_freed_gb']:.2f}GB"
        )
        return 0
    except Exception as e:
