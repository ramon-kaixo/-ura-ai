...
class SystemCleaner:
    ...
    def record_operation(self, operation: str, size_freed: float):
        ...
        self.operations.append(
            {"operation": operation, "size_freed_gb": size_freed, "timestamp": datetime.now().isoformat()}
        )
...
class LinuxCleaner(SystemCleaner):
    ...
    def clean_pip_cache(self) -> float:
        ...
        try:
            ...
            result = subprocess.run(["pip3", "cache", "purge"], capture_output=True, text=True, timeout=300)
...
class MacCleaner(SystemCleaner):
    ...
    def clean_docker(self) -> float:
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "system", "prune", "-a", "--volumes", "-f"], capture_output=True, text=True, timeout=300
            )
    ...
    def clean_brew_cache(self) -> float:
        ...
        try:
            ...
            result = subprocess.run(["brew", "cleanup", "--prune=all"], capture_output=True, text=True, timeout=300)
