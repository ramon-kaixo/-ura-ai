...
def run_remote_maintenance(ip: str, user: str = None) -> dict:
    ...
    if not validate_ip(ip):
        return {"ip": ip, "status": "error", "error": "Invalid IP address format"}

    if not validate_ssh_user(user):
        return {"ip": ip, "status": "error", "error": "Invalid SSH username"}
    ...
    try:
        ...
        if not os.path.exists(local_script):
            return {"ip": ip, "status": "error", "error": f"Local script not found: {local_script}"}

        try:
            subprocess.run(
                ["scp", local_script, f"{user}@{ip}:{remote_script}"], check=True, timeout=CONFIG["ssh_timeout"]
            )
        except subprocess.TimeoutExpired:
            return {"ip": ip, "status": "error", "error": "SCP timeout"}
        except subprocess.CalledProcessError as e:
            return {"ip": ip, "status": "error", "error": f"SCP failed: {e}"}
        except FileNotFoundError:
            return {"ip": ip, "status": "error", "error": "SCP command not found"}
        ...
        try:
            ...
            return {"ip": ip, "status": "error", "error": "SSH execution timeout"}
        except subprocess.CalledProcessError as e:
            return {"ip": ip, "status": "error", "error": f"SSH execution failed: {e}"}
        except FileNotFoundError:
            return {"ip": ip, "status": "error", "error": "SSH command not found"}

        if result.returncode == 0:
            logger.info(f"Mantenimiento completado en {ip}")
            return {"ip": ip, "status": "success", "output": result.stdout}
        else:
            logger.error(f"Error en mantenimiento en {ip}: {result.stderr}")
            return {"ip": ip, "status": "error", "error": result.stderr}
