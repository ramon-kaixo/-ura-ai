...
def cmd_ask(args):
    ...
    question_b64 = base64.b64encode(question.encode()).decode()
    script = (
        "import base64, sys\n"
        "from core.memory_engine import query\n"
        "q = base64.b64decode(sys.argv[1]).decode()\n"
        "for x in query(q):\n"
        '  print("[{id}] ({score:.2f}) {text}".format(\n'
        '    id=x.get("id","?"), score=x.get("score",0),'
        '    text=str(x.get("text",""))[:200]))\n'
    )
    remote = f"cd ~/URA/ura_ia_1972 && python3 -c {shlex.quote(script)} {shlex.quote(question_b64)}"
    return subprocess.run(["ssh", TARGET, remote], cwd=ROOT, timeout=30, check=False).returncode
