...
def _ok(v): return f"{G}✅{E}" if v else f"{R}❌{E}"
def _val(v): return f"{Y}{v}{E}"
def _svc(n):
...
def main():
    ...
    if tr.returncode != 0:
        print(f"\n  {R}UNIT TESTS FAILED{E}"); sys.exit(1)
    for mn, mp in [("modules_ingest","core/modules/ingest/coordinator.py"),("modules_ai","core/modules/ai/model_broker.py"),("modules_infra","core/modules/infra/action_handler.py")]:
        ok = (REPO_DIR / mp).exists(); check(mn, ok, "exists" if ok else "missing", "exists")
    pd = REPO_DIR / "data" / "processed"
    try:
        pd.mkdir(parents=True,exist_ok=True); tf=pd/".write_test"; tf.write_text("ok"); tf.unlink()
        check("data_processed", True, "writable", "writable")
    except Exception as e: check("data_processed", False, str(e), "writable")
    for comp in required: ok = _svc(comp); check(f"service:{comp}", ok, "active" if ok else "inactive", "active")
    try:
        ctx = zmq.Context(); sock = ctx.socket(zmq.REQ); sock.setsockopt(zmq.RCVTIMEO, 5000)
        sock.connect("ipc:///tmp/ura-supervisor.ipc"); sock.send(b"tasks"); tasks = json.loads(sock.recv()); sock.close(); ctx.term()
    ...
    except Exception as e: check("latency", False, f"error: {e}", f"<={metrics['latency_max_ms']}ms")
    try:
    ...
    except Exception: check("chaos_tests", False, "error", "passed")
    ok_all = failed == 0
    if as_json: print(json.dumps({"version":ver,"all_ok":ok_all,"passed":passed,"failed":failed,"total":len(results),"checks":results}, indent=2))
    elif not quiet:
        print(f"\n{B}{'='*58}{E}\n{B}  VALIDACI\u00d3N DE BASELINE v{ver}{E}\n{B}{'='*58}{E}\n")
        for c in results:
