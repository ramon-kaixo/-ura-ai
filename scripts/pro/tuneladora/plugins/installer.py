...
class InstallerPlugin:
    ...
    def check_requirements(self) -> dict[str, Any]:
        ...
        # pip
        try:
            subprocess.run([sys.executable, "-m", "pip", "--version"], capture_output=True, timeout=10, check=False)  # nosec
            results["pip"] = {"ok": True}
        except Exception:
            results["pip"] = {"ok": False, "error": "pip no disponible"}
        # git
        try:
            subprocess.run(["git", "--version"], capture_output=True, timeout=10, check=False)  # nosec
            results["git"] = {"ok": True}
