...
if command -v git >/dev/null 2>&1 && [ -d "$REPO/.git" ] && [ "${SKIP_MERGE:-}" != "1" ]; then
	if git show-ref --verify --quiet refs/heads/mac-veredictos 2>/dev/null; then
		...
		if flock -n 9 2>/dev/null; then
			...
			elif [ "$(git rev-list --count HEAD..mac-veredictos 2>/dev/null)" -le 10 ] &&
				[ "$(git diff --name-only HEAD mac-veredictos 2>/dev/null | wc -l)" -le 50 ]; then
				# Rama divergida PERO con delta acotado (≤10 commits, ≤50 archivos):
				# merge no-ff acotado y seguro (veredictos UDO). Evita merges
				# destructivos masivos (auditoría 2026-08-28: 1036 archivos/287k
				# eliminaciones al intentar integrar mac-veredictos divergida).
				if HOME=/tmp PRE_COMMIT_HOME=/tmp/opencode/precommit3 SKIP=semgrep,pytest \
					git merge mac-veredictos --no-ff -m "chore(udo): integrar veredictos del Mac (auto-integracion detector)" 2>/dev/null; then
					echo "VEREDICTOS_MAC: integrados (delta acotado ≤10 commits/≤50 archivos)"
				else
					git merge --abort 2>/dev/null || true
					echo "AVISO: merge de mac-veredictos falló (revisar conflictos)"
				fi
			else
				echo "AVISO: mac-veredictos divergida en exceso (delta >10 commits o >50 archivos)."
				echo "       Integración DIFERIDA: requiere revisión humana (no merge automático destructivo)."
			fi
...
for f in "$TASKS_DIR"/*.md; do
	[ -f "$f" ] || continue
	est=$(grep '^estado:' "$f" 2>/dev/null | cut -d' ' -f2- | tr -d ' ' || true)
