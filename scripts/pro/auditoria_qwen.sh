...
log "Verificando modelo qwen3-coder:30b..."
if ! ollama ps 2>/dev/null | grep -q "qwen3-coder:30b"; then
...
for mod in core/memory_engine.py core/auth_layer.py; do
    if [ -f "$mod" ]; then
        ...
        RESPUESTA=$(ollama run qwen3-coder:30b "$PROMPT
...
echo "{\"fecha\":\"$FECHA\",\"modulos_analizados\":[\"core/memory_engine.py\",\"core/auth_layer.py\"],\"resultado\":\"completado\"}" > "$REPORTE"
