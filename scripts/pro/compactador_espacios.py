    ...
    for i, linea in enumerate(lineas):
        ...
        if en_docstring:
            if delimiter_docstring in stripped:
                en_docstring = False
            stats["docstrings"] += 1
            anchors.append({"original": num_original, "tipo": "docstring", "texto": linea})
            continue
...
def descompactar(codigo_compactado: str, anchors: list) -> str:
    ...
    for anchor in anchors:
        ...
        elif anchor["tipo"] == "blanco":
            lineas_originales.append("")
        elif anchor["tipo"] == "comentario_inline":
            # El compactado ya contiene la línea sin el comentario inline;
            # se re-añade el comentario al final de esa línea (no emite línea nueva).
            texto = anchor.get("texto", "")
            if lineas_originales and texto:
                idx_com = texto.find("#")
                if idx_com >= 0:
                    lineas_originales[-1] = lineas_originales[-1].rstrip() + " " + texto[idx_com:]
        elif anchor["tipo"] in ("comentario", "docstring"):
            # Restaurar el contenido original guardado en el anchor (fix TASK-20260812-017:
            # antes se perdía el texto de comentarios y docstrings).
            lineas_originales.append(anchor.get("texto", ""))
