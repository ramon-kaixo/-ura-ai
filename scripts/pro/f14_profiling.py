...
class Snapshotter:
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "rss_mb": {"min": min(rss_vals), "max": max(rss_vals),
            ...
            "num_threads": {"min": min(thr_vals), "max": max(thr_vals),
...
def load_task(n: int):
    ...
    try:
        ...
        queries = ["qué es URA", "arquitectura del sistema", "memoria episódica",
                   "consenso multi-agente", "plugin registry", "Qdrant retrieval",
                   "FSM de conciencia", "degraded mode", "fragmentos de conocimiento",
...
def scenario_p01(snap: Snapshotter, duration_s: int = 300) -> dict:
    ...
    if rss_growth > summary["rss_mb"]["mean"] * 0.05:
        ...
        record_finding("P01", f"RSS creció {rss_growth:.1f}MB en reposo ({duration_s}s) sin carga. Posible leak basal.", "medio")
    if thread_stable > 2:
...
def scenario_p02(snap: Snapshotter, duration_s: int = 600) -> dict:
    ...
    if growth_rate > 5:
        ...
        record_finding("P02", f"RSS creció {growth_rate:.1f}MB/min durante carga media. Posible memory leak bajo carga.", "alto")
    thread_leak = summary["num_threads"]["max"] - summary["num_threads"]["min"]
    if thread_leak > 5:
        anomalies.append(f"Hilos crecieron {thread_leak} durante carga")
        record_finding("P02", f"Número de hilos aumentó en {thread_leak} durante carga media. Posible thread leak.", "alto")
    verdict = "PASS" if not anomalies else "PARTIAL"
    return {
        "id": "P02",
...
def scenario_p03(snap: Snapshotter, duration_s: int = 180) -> dict:
    ...
    if summary["rss_mb"]["max"] > summary["rss_mb"]["min"] * 1.3:
        ...
        record_finding("P03", f"RSS creció {growth:.1f}MB durante carga pico ({duration_s}s). Posible presión de memoria.", "medio")
    verdict = "PASS" if not anomalies else "PARTIAL"
    return {
        "id": "P03",
...
def scenario_p04(snap: Snapshotter, duration_s: int = 300) -> dict:
    ...
    if rss_last > rss_first * 1.1 and rss_last > rss_first + 10:
        ...
        record_finding("P04", f"RSS no retornó a nivel basal tras carga. Primera muestra={rss_first:.0f}MB, última={rss_last:.0f}MB, pico={rss_peak:.0f}MB.", "alto")
    verdict = "PASS" if not anomalies else "PARTIAL"
    return {
        "id": "P04",
...
def scenario_p05(snap: Snapshotter, cycles: int = 3, cycle_duration_s: int = 180) -> dict:
    ...
    if rss_first_cycle and rss_last_cycle and rss_last_cycle > rss_first_cycle * 1.1:
        ...
        record_finding("P05", f"Fatiga de recursos: RSS creció de {rss_first_cycle:.0f}MB (ciclo 1) a {rss_last_cycle:.0f}MB (ciclo {cycles}). Posible leak acumulativo.", "alto")
    verdict = "PASS" if not anomalies else "PARTIAL"
    return {
        "id": "P05",
...
def save_results(results: list[dict], env: dict):
    ...
    with open(csv_path, "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=["id", "veredict", "duration_s",
                                          "rss_min", "rss_max", "rss_mean", "rss_p95",
                                          "cpu_min", "cpu_max", "cpu_mean",
                                          "threads_min", "threads_max", "threads_mean",
                                          "anomalies_count"])
        w.writeheader()
        for r in results:
            s = r.get("summary", {})
...
def main():
    ...
    parser.add_argument("--scenarios", nargs="*", choices=["P01","P02","P03","P04","P05"],
                        help="Escenarios específicos (defecto: todos)")
    parser.add_argument("--fast", action="store_true",
                        help="Usa duraciones reducidas para test rápido")
