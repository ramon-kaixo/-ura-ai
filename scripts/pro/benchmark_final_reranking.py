...
def run(name, retriever_fn, queries, rel_map, reranker=None):
    ...
    for q in queries:
        ...
        R["Recall@10"].append(r10); R["Precision@5"].append(p5)
        R["MRR"].append(mrr); R["MAP"].append(ap); R["nDCG@10"].append(ndcg)
    sl = sorted(lat); n = len(sl)
...
def main():
    ...
    cfg = UraConfig(); qc = QdrantClient.instancia(cfg)
    if not qc.disponible:
        print("Qdrant not available"); return 1

    queries, rel_map = load()
    vec = VectorRetriever(qc); lex = LexicalRetriever()
    ...
    metrics_list = ["Recall@10", "Precision@5", "MRR", "MAP", "nDCG@10", "P50", "P95", "P99", "Throughput", "No-context"]
    ...
    for name in configs:
        r = results[name]
        print(f"  {name:<28} {r['Recall@10']:<8.4f} {r['Precision@5']:<8.4f} {r['MRR']:<8.4f} {r['MAP']:<8.4f} {r['nDCG@10']:<8.4f} {r['P50']:<8.2f} {r['P95']:<8.2f} {r['P99']:<8.2f} {r['Throughput']:<8.2f} {r['No-context']:<8.2%}")
    ...
    checks = [
        ...
        ("nDCG >= Vector-only", r["nDCG@10"] >= b["Vector only"]["nDCG"], f"{r['nDCG@10']:.4f} >= {b['Vector only']['nDCG']:.4f}"),
        ("R@10 >= Hybrid", r["Recall@10"] >= b["Hybrid"]["R@10"], f"{r['Recall@10']:.4f} >= {b['Hybrid']['R@10']:.4f}"),
        ("NoCtx <= Hybrid", r["No-context"] <= b["Hybrid"]["NoCtx"], f"{r['No-context']:.2%} <= {b['Hybrid']['NoCtx']:.2%}"),
        ("P95 <= Hybrid +25%", r["P95"] <= b["Hybrid"]["P95"] * 1.25, f"{r['P95']:.2f} <= {b['Hybrid']['P95'] * 1.25:.2f}"),
    ]
    all_pass = True
    ...
    print(f"{'='*90}")
    print(f"  {'Vector-only:':<30} R@10={results['Vector-only']['Recall@10']:.4f} MAP={results['Vector-only']['MAP']:.4f} P95={results['Vector-only']['P95']:.0f}ms")
    print(f"  {'Hybrid:':<30} R@10={results['Hybrid (score)']['Recall@10']:.4f} MAP={results['Hybrid (score)']['MAP']:.4f} P95={results['Hybrid (score)']['P95']:.0f}ms")
    print(f"  {'Hybrid + CE:':<30} R@10={results['Hybrid + CrossEncoder']['Recall@10']:.4f} MAP={results['Hybrid + CrossEncoder']['MAP']:.4f} P95={results['Hybrid + CrossEncoder']['P95']:.0f}ms")
    if all_pass:
