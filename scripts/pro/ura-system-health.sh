...
# Modelos esperados en el Ollama de GX10 (fuente de verdad de modelos de trabajo).
# Se verifica conectividad a /api/tags, el conteo total y la presencia del
# modelo principal (qwen3-coder:30b-mejorado). Si el conteo baja o el modelo
# principal falta, es fallo: la config de OpenCode depende de ellos.
REQUIRED_OLLAMA_MODEL="qwen3-coder:30b-mejorado"
MIN_OLLAMA_MODELS=9

check_ollama_models() {
	echo "-- Ollama Models (11434) --"
	local tags
	tags=$(curl -sf --max-time 8 http://localhost:11434/api/tags 2>/dev/null) || {
		log_fail "Ollama /api/tags no responde (conectividad)"
		return
	}
	local count
	count=$(echo "$tags" | python3 -c "import sys,json;print(len(json.load(sys.stdin).get('models',[])))" 2>/dev/null)
	if [ -z "$count" ]; then
		log_fail "Ollama /api/tags respuesta invalida"
		return
	fi
	if echo "$tags" | python3 -c "import sys,json;d=json.load(sys.stdin);sys.exit(0 if '$REQUIRED_OLLAMA_MODEL' in [m['name'] for m in d.get('models',[])] else 1)" 2>/dev/null; then
		log_pass "modelo $REQUIRED_OLLAMA_MODEL presente"
	else
		log_fail "modelo $REQUIRED_OLLAMA_MODEL NO presente"
	fi
	if [ "$count" -ge "$MIN_OLLAMA_MODELS" ]; then
		log_pass "$count modelos (min $MIN_OLLAMA_MODELS)"
	else
		log_fail "$count modelos < minimo $MIN_OLLAMA_MODELS"
	fi
}
...
check_nvidia() {
	...
	local drv temp pwr
	drv=$(nvidia-smi --query-gpu=driver_version --format=csv,noheader 2>&1) || {
		log_fail "nvidia-smi failed"
		return
	}
...
check_modules() {
	...
	for mod in nvidia nvidia_modeset nvidia_drm nvidia_uvm; do
		if lsmod 2>/dev/null | grep -q "^$mod "; then :; else
			log_fail "Module $mod not loaded"
			((m++))
		fi
	done
