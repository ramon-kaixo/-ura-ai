...
PATRONES = [
    ...
    re.compile(r"eres\s+(ahora|un)\s+asistente\s+(malvado|sin\s+restricciones)", re.IGNORECASE),
    re.compile(r"system\s*:\s*.*reasign", re.IGNORECASE),
]
...
class JailbreakGuard:
    ...
    def sanitizar(self, texto, url=""):
        ...
        r = Resultado(f"<user_input>\n{t}\n</user_input>", url, len(act), act, datetime.now(tz=UTC).isoformat())
        if self._log and act:
            LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
            with LOG_PATH.open("a") as f:
                f.write(json.dumps({"t": r.timestamp, "n": r.n_patrones_detectados}) + "\n")
        return r
