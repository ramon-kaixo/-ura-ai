...
check("TC4: auditor is qwen2.5:3b-instruct",
      lambda: cfg["models"]["auditor"]["name"] == "qwen2.5:3b-instruct")
check("TC4: plan_path = /tmp/ura_debate_plan.json",
      lambda: cfg.get("plan_path") == "/tmp/ura_debate_plan.json")
...
async def _test_call_ollama_timeout():
    ...
    result = await call_ollama(client, "http://nonexistent:11434", "test-model",
                               "test prompt", timeout=0.001)
    return result


check("TC5: call_ollama timeout returns None",
      lambda: asyncio.run(_test_call_ollama_timeout()) is None)
...
async def _test_call_ollama_json_decode_error():
    ...
    client = httpx.AsyncClient()
    result = await call_ollama(client, "http://test:11434", "test-model",
                               "test prompt", timeout=5)
    return result
...
check("TC6: call_ollama bad JSON returns None",
      lambda: asyncio.run(_test_ollama_not_json()) is None)


async def _test_ollama_markdown_json():
    """Respuesta con markdown code block debe parsearse igual"""
    transport = MockTransport(json.dumps({
...
check("TC7: call_ollama parses markdown JSON block",
...
check("TC8: call_ollama parses plain JSON",
...
async def _run_debate_mocked(primary_resp: dict, auditor_resp: dict,
...
async def _direct_run_debate_test(primary: dict | None, auditor: dict | None,
                                   threshold: float = 0.85) -> dict:
    ...
    if primary is None or auditor is None:
        ...
        if primary and primary.get("suggestions"):
            plan_unified += "\n\n# Mejoras sugeridas:\n" + "\n".join(
                f"- {s}" for s in primary["suggestions"][:3]
            )
    else:
...
r_cons = asyncio.run(_direct_run_debate_test(
    _make_primary_response(0.9, ["riesgo menor"], ["usar logging"]),
    _make_auditor_response(0.85, ["riesgo menor"]),
))
check("TC9: consensus verdict = CONSENSUS",
      lambda: r_cons["verdict"] == "CONSENSUS")
check("TC9: consensus >= 0.85",
      lambda: r_cons["consensus"] >= 0.85)
check("TC9: consensus primary_risks preserved",
      lambda: "riesgo menor" in r_cons["primary_risks"])
check("TC9: consensus auditor_risks preserved",
      lambda: "riesgo menor" in r_cons["auditor_risks"])
check("TC9: consensus plan has suggestions",
      lambda: "Mejoras sugeridas" in r_cons["plan_unified"])

# TC10: HUMAN_ARBITRATION — consensus below threshold
r_low = asyncio.run(_direct_run_debate_test(
    _make_primary_response(0.6, ["riesgo grave"]),
    _make_auditor_response(0.5, ["fallo critico"]),
))
check("TC10: low consensus verdict = HUMAN_ARBITRATION",
      lambda: r_low["verdict"] == "HUMAN_ARBITRATION")
check("TC10: low consensus < 0.85",
      lambda: r_low["consensus"] < 0.85)
check("TC10: low consensus score preserved",
      lambda: r_low["consensus"] == 0.5)

# TC11: HUMAN_ARBITRATION — auditor requires_human even with high scores
r_human = asyncio.run(_direct_run_debate_test(
    _make_primary_response(0.95),
    _make_auditor_response(0.9, ["race condition", "vram overflow"], requires_human=True),
))
check("TC11: requires_human verdict = HUMAN_ARBITRATION",
      lambda: r_human["verdict"] == "HUMAN_ARBITRATION")
check("TC11: requires_human threshold >0.7 despite high scores",
      lambda: r_human["consensus"] >= 0.85)

# TC12: INCOMPLETE — primary returns None
r_inc = asyncio.run(_direct_run_debate_test(
    None,
    _make_auditor_response(0.5),
))
check("TC12: None primary verdict = INCOMPLETE",
      lambda: r_inc["verdict"] == "INCOMPLETE")
check("TC12: INCOMPLETE primary_reason = timeout/error",
      lambda: r_inc["primary_reason"] == "timeout/error")

# TC13: INCOMPLETE — auditor returns None
r_inc2 = asyncio.run(_direct_run_debate_test(
    _make_primary_response(0.9),
    None,
))
check("TC13: None auditor verdict = INCOMPLETE",
      lambda: r_inc2["verdict"] == "INCOMPLETE")
check("TC13: INCOMPLETE auditor_reason = timeout/error",
      lambda: r_inc2["auditor_reason"] == "timeout/error")

# TC14: INCOMPLETE — both None
r_inc3 = asyncio.run(_direct_run_debate_test(None, None))
check("TC14: both None verdict = INCOMPLETE",
      lambda: r_inc3["verdict"] == "INCOMPLETE")

# TC15: consensus exact boundary (score == threshold)
r_bound = asyncio.run(_direct_run_debate_test(
    _make_primary_response(0.85),
    _make_auditor_response(0.85),
    threshold=0.85,
))
check("TC15: exact threshold verdict = CONSENSUS",
      lambda: r_bound["verdict"] == "CONSENSUS")
check("TC15: exact threshold = 0.85",
      lambda: r_bound["consensus"] == 0.85)

# TC16: plan_unified without suggestions
r_no_sugg = asyncio.run(_direct_run_debate_test(
    _make_primary_response(0.9, risks=[], suggestions=None),
    _make_auditor_response(0.9),
))
check("TC16: no suggestions plan is plain text",
      lambda: r_no_sugg["plan_unified"] == "test plan")
check("TC16: no suggestions still CONSENSUS",
      lambda: r_no_sugg["verdict"] == "CONSENSUS")


# ============================================================
# SECTION 5: plan_validator — state files + format + exit codes
# ============================================================
from core.debate.plan_validator import load_state_file, format_context_for_prompt, collect_context
...
check("TF1: load_state_file reads valid JSON",
      lambda: state is not None and state.get("mode") == "EMERGENCY")
os.unlink(tf.name)

# TF2: load_state_file — non-existent file
check("TF2: load_state_file returns None for missing file",
...
tf3.close()
check("TF3: load_state_file returns None for bad JSON",
      lambda: load_state_file(tf3.name) is None)
os.unlink(tf3.name)
...
check("TF4: formatted includes service names",
      lambda: "test.service" in formatted and "dead.service" in formatted)
check("TF4: formatted includes PID for active",
      lambda: "PID 1234" in formatted)
check("TF4: formatted includes SNC mode",
      lambda: "NORMAL" in formatted)
check("TF4: formatted includes VRAM error",
      lambda: "nvidia-smi no disponible" in formatted)
check("TF4: formatted includes timestamp",
      lambda: "Timestamp:" in formatted)
...
check("TF5: formatted includes VRAM total",
      lambda: "128000 MB" in fv)
check("TF5: formatted includes VRAM used %",
      lambda: "50.0%" in fv or "50%" in fv)
check("TF5: formatted includes free VRAM",
      lambda: "64000 MB" in fv)

# TF6: collect_context returns dict with services (even if services are unknown)
ctx_real = collect_context()
check("TF6: collect_context returns dict",
      lambda: isinstance(ctx_real, dict))
check("TF6: collect_context has services",
      lambda: "services" in ctx_real)
check("TF6: collect_context has vram",
      lambda: "vram" in ctx_real)
check("TF6: services is a list",
      lambda: isinstance(ctx_real["services"], list))
check("TF6: each service has name/active keys",
...
check("TF7: CONSENSUS → exit 0",
      lambda: _simulate_main({"verdict": "CONSENSUS"}) == 0)
check("TF7: HUMAN_ARBITRATION → exit 2",
      lambda: _simulate_main({"verdict": "HUMAN_ARBITRATION"}) == 2)
check("TF7: INCOMPLETE → exit 1",
      lambda: _simulate_main({"verdict": "INCOMPLETE"}) == 1)
check("TF7: empty verdict → exit 1",
      lambda: _simulate_main({"verdict": ""}) == 1)
check("TF7: missing verdict → exit 1",
      lambda: _simulate_main({}) == 1)
...
check("TE1: empty plan primary prompt works",
      lambda: "CONTEXTO DEL SISTEMA" in p_empty)
a_empty = build_auditor_prompt("")
check("TE1: empty plan auditor prompt works",
      lambda: "ABOGADO DEL DIABLO" in a_empty)
...
check("TE3: extra context keys don't break formatting",
      lambda: "x.service" in fe)

# TE4: Load config validates committee_config.json is correct
cfg2 = load_config()
check("TE4: config timeout_per_model >= 30",
      lambda: cfg2["timeout_per_model"] >= 30)
check("TE4: config primary temperature <= 0.3",
      lambda: cfg2["models"]["primary"]["temperature"] <= 0.3)
check("TE4: config auditor temperature > primary",
...
dl_idem.acquire()
dl_idem.release()
check("TE5: double release no-crash",
      lambda: (dl_idem.release(), True)[1])
check("TE5: file removed after double release",
      lambda: not os.path.exists("/tmp/test_lock_idem.lock"))
