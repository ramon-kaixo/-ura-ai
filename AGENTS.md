...
- **CLI**: `scripts/pro/ura-udo` — `create | show | update (--estado/--nota/--reserva/--agente_web/--agente_terminal) | reserve (--add/--clear) | check [rutas] | list [ESTADO] | status | verify TASK-ID`
- **Reserva de archivos (F2)**: `reserve TASK --add "ruta1,ruta2"` declara qué archivos tocará la tarea; `check ruta...` detecta CONFLICTO contra reservas activas (IN_PROGRESS/REVIEW, match exacto o prefijo `dir/`). Persistente en el expediente (Git). Liberación automática al cierre. `commit_base` automático al IN_PROGRESS (también en tareas F1 ya IN_PROGRESS).
