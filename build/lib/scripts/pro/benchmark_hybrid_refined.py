...
CHUNKING_BASELINE = {
    "Recall@1": 0.1250, "Recall@5": 0.4567, "Recall@10": 0.6700,
    "Precision@5": 0.4750, "MRR": 0.7595, "MAP": 0.9423, "nDCG@10": 0.8346,
    "Latency P50 (ms)": 90.99, "Latency P95 (ms)": 195.57,
    "Latency P99 (ms)": 368.46, "Throughput (qps)": 9.82, "No-context rate": 0.215,
}
...
def _fuse_weighted(vr, lr, alpha, beta):
    ...
    for r in vr:
        ...
        score_map.setdefault(r["doc_id"], {"doc_id": r["doc_id"], "vector_score": 0.0, "lexical_score": 0.0, "vector_rank": 999, "lexical_rank": 999})
...
def main():
    ...
    if not qc.disponible:
        log.error("Qdrant not available"); return 1
    ...
    for name, strategy_fn in STRATEGIES.items():
        ...
        for q in queries:
            ...
            all_r1.append(m[0]); all_r5.append(m[1]); all_r10.append(m[2])
            all_p5.append(m[3]); all_mrr.append(m[4]); all_ndcg.append(m[5]); all_ap.append(m[6])
    ...
    print(f"\n{'='*120}")
    print(f"  {'Strategy':<32} {'R@10':<8} {'MRR':<8} {'MAP':<8} {'nDCG':<8} {'P50':<8} {'P95':<8} {'TPS':<8} {'NoCtx':<8} {'Pass':<6}")
    print(f"{'='*120}")
    ...
    for name, r in sorted(results.items()):
        p = passes(name, r)
        flag = "✅" if p else "❌"
        print(f"  {name:<32} {r['Recall@10']:<8.4f} {r['MRR']:<8.4f} {r['MAP']:<8.4f} {r['nDCG@10']:<8.4f} {r['P50']:<8.2f} {r['P95']:<8.2f} {r['Throughput']:<8.2f} {r['No-context']:<8.2%} {flag:<6}")
        if p:
    ...
    if best:
        ...
        # Show closest
        best_all = max(results.keys(), key=lambda n: results[n]["Recall@10"] + results[n]["MAP"] + results[n]["nDCG@10"])
