...
def load_queries():
    ...
    with open(CORPUS_DIR / "relevance.jsonl") as f:
        for l in f:
            d = json.loads(l); rel.setdefault(d["qid"], []).append(d)
    return queries, rel
...
def compute(retrieved, gold_set, gold_rel, k=10):
    ...
    for i, d in enumerate(ids):
        if d in gold_set: hits += 1; ap_sum += hits / (i + 1)
...
def run(name, fn, queries, rel_map, reranker=None):
    ...
    for q in queries:
        ...
        score = results[0].get("reranker_score", 0) if results and reranker else (results[0].get("score", 0) if results else 0)
        if not results or score < 0.6: no_ctx += 1
        m = compute(results, gs, gr)
        all_r10.append(m[0]); all_p5.append(m[1]); all_mrr.append(m[2]); all_ndcg.append(m[3]); all_ap.append(m[4])
    slat = sorted(all_lat); n = len(slat)
    if ce_err:
        log.warning("  CrossEncoder errors: %d/%d", ce_err, len(queries))
    return {
        "R@10": round(statistics.mean(all_r10), 4), "P@5": round(statistics.mean(all_p5), 4),
        "MRR": round(statistics.mean(all_mrr), 4), "MAP": round(statistics.mean(all_ap), 4),
...
def main():
    ...
    if not qc.disponible: log.error("Qdrant not available"); return 1
    ...
    all_results["Hybrid+CE"] = run("Hybrid+CE", lambda q, k: hybrid.search(q, k), queries, rel_map, reranker=ce_reranker)

    # Print table
    metrics = ["R@10", "P@5", "MRR", "MAP", "nDCG", "P50", "P95", "P99", "TPS", "NoCtx"]
    print(f"\n{'='*120}")
    print(f"  {'Config':<20} {'R@10':<8} {'P@5':<8} {'MRR':<8} {'MAP':<8} {'nDCG':<8} {'P50':<8} {'P95':<8} {'P99':<8} {'TPS':<8} {'NoCtx':<8}")
    print(f"{'='*120}")
    for name in ["Vector-only", "Hybrid", "Hybrid+CE"]:
        r = all_results.get(name)
        if r: print(f"  {name:<20} {r['R@10']:<8.4f} {r['P@5']:<8.4f} {r['MRR']:<8.4f} {r['MAP']:<8.4f} {r['nDCG']:<8.4f} {r['P50']:<8.2f} {r['P95']:<8.2f} {r['P99']:<8.2f} {r['TPS']:<8.2f} {r['NoCtx']:<8.2%}")
