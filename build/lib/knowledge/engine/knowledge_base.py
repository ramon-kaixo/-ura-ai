...
"""
        ...
        for doc_type in sorted(by_type.keys()):
            ...
            for doc in docs:
                ...
                if prev_manifest.get(doc_id) == new_hash:
                    ...
                    nav_entry[doc_type].append({doc["title"]: f"{_sanitize_filename(doc_type)}/{_sanitize_filename(doc_id)}.md"})
                    count += 1
                    continue
        ...
        (dest / "mkdocs.yml").write_text(yaml.dump(mkdocs_config, default_flow_style=False, sort_keys=True), encoding="utf-8")

        # 6. Generar index.md
        index_lines = ["# Knowledge Base\n", f"Generated from Knowledge Engine v0.2.0\n", f"**{count} documents**\n", "\n## Categories\n"]
        for doc_type in sorted(by_type.keys()):
